...
<body>
    <div id="app">
        <div class="container py-2 d-flex mb-4">
            <a class="d-flex" href="/">
                <div class="mr-3">
                    <img src="{{ asset(config('constants.nep_gov.logo_sm')) }}" alt="Nepal Government" style="max-height: 90px;">
                </div>
                <div class="d-flex flex-column align-self-center">
                    <h4 class="h4-responsive text-danger font-weight-bold">{{ settings('municipality_name') }}</h4>
                    <h5 class="h5-responsive text-danger">{{ settings('municipality_tagline') }}</h5>
                </div>
            </a>
            <div class="ml-auto align-self-center">
                @guest
                <a id="login-link" class="py-2 px-3 rounded font-noto" href="{{ route('login') }}">Login</a>
                @endguest
            </div>
        </div>
        @yield('content')
