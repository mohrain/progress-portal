<template>
  <div class="mt-4">
    <CoolLightBox :items="photos" :index="index" :fullscreen="true" @close="index = null">
    </CoolLightBox>
    <div class="box mt-3">
      ...
      <div class="box__body">
        <section v-if="photos.length" class="gallery-grid">
          <div v-for="(photo, index) in photos" v-bind:key="photo.id"
            v-bind:class="{ 'photo-box': true, active: selectedPhoto && selectedPhoto.id == photo.id }"
...
export default {
  ...
  methods: {
    fetchPhotos() {
      ...
      axios.get(`/gallery/${this.album.id}/getPhotos`)
        .then(response => {
